...
export default function WhatsAppButton() {
  const whatsappUrl =
    "https://wa.me/5491127887093?text=Hola%2C%20quiero%20cotizar%20un%20sitio%20web%20para%20mi%20negocio.";
  ...
  return (
    <aside aria-label="Contacto directo por WhatsApp">
      <a
        ...
        aria-label="Cotiza por WhatsApp con Alan López"
        title="Cotiza por WhatsApp"
        className="floating-whatsapp-container fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-40 flex items-center gap-2.5 sm:gap-3 cursor-pointer no-underline select-none group focus:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2 focus-visible:ring-offset-liquid-abyss rounded-full"
      >
        {/* Tooltip on Mobile (revealed on hover / focus to avoid covering content) */}
        <span className="floating-whatsapp-badge sm:hidden absolute right-full mr-2.5 pointer-events-none opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 group-focus:opacity-100 group-focus:translate-x-0 transition-all duration-200 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-liquid-deep/95 border border-white/10 backdrop-blur-md text-platinum text-[11px] font-matter font-medium whitespace-nowrap">
        ...
        {/* Discreet Badge on Desktop (always visible beside button) */}
        ...
        <div
          className="floating-whatsapp-btn w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#25D366] flex items-center justify-center text-white shrink-0"
        >
          <svg
